...
import com.example.cmu.encryption.AES256Cipher;
...
public class Reservation extends Activity {
    ...
    private Send_Resv_Info rio;
    private NetworkService ns;
    private AsyncReservation aResv;
    private SharedPreferences pref;
    private SharedPreferences.Editor editor;

    private int status = 0; //whether reservation is success or not
    private int resvHour = 0; // current reservation hour
    private int resvMin = 0; // current reservation minute
    private String pNum = ""; // user phone_number
    private String cNum = ""; // user card_number

    private MainActivity tabs = null;

    //connect to push server
    String pushServerIp = "192.168.1.166";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        spinner_facility = (Spinner) findViewById(R.id.spinner);

        aes = new AES256Cipher();

        ArrayAdapter adapter1 = ArrayAdapter.createFromResource(this, R.array.Parking_Facility, android.R.layout.simple_spinner_dropdown_item);
        adapter1.setDropDownViewResource(android.R.layout.simple_dropdown_item_1line);
        spinner_facility.setAdapter(adapter1);
        ...
        submit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (reservedTime - currentTime > 180 || reservedTime - currentTime < 0) {
                    ...
                    try {
                        //Encryption
                        pNum = aes.AES_Encode(phoneNum.getText().toString());
                        cNum = aes.AES_Encode(credit.getText().toString());
                        Log.d("abc", "Reservation encryption:" + pNum + "\n" + cNum);
                    } catch (Exception e) {
                        e.printStackTrace();
                        Log.d("abc", "Reservation encryption IN Catch");
                    }
                    rio.setResv_phonenum(pNum);
                    rio.setResv_creditnum(cNum);
                    rio.setResv_starttime(String.valueOf(resvHour) + ":" + String.valueOf(resvMin));
    ...
    private AlertDialog createDialog(int status) {
        ...
        if (status == 0) {
            ...
            ab.setMessage("Connection failed: Server down");
        }
    ...
    public class AsyncReservation extends AsyncTask<String, Integer, String> {
        ...
        protected void onPostExecute(String str) {
            ...
            try {
                //Log.i("abc", "AsyncReservation, OnPost:" + aes.AES_Decode(rri.getResv_authenticationnum()));
                ...
                if (!"null".equals(str)) {
                    ...
                    editor.putString("authen_num", aes.AES_Decode(rri.getResv_authenticationnum()));
                    Log.d("abc", "encrypted authen_num" + rri.getResv_authenticationnum());
                    ...
                    String clientId = "client" + aes.AES_Decode(rri.getResv_authenticationnum());
                    MQTTUtils.connect(pushServerIp, clientId, getApplicationContext());

                    //register topic value on authentication_number for receiving push alarm
                    String topic = "topic/" + aes.AES_Decode(rri.getResv_authenticationnum());
                    MQTTUtils.sub(topic);

                    Log.i("abc", "clientId: " + clientId + "   topic:" + topic);
